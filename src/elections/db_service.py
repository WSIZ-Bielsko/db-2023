...
class DbService:
    ...
    async def upsert_election_type(self, election_type: ElectionType) -> ElectionType:
        ...
        for retry in range(NO_OF_RETRIES):
            async with self.pool.acquire() as connection:
                try:
                    async with connection.transaction(isolation=TRANSACTION_MODE):
                        if et.type_id is None:
                            row = await connection.fetchrow("""INSERT INTO election_type(type_name, no_of_choices) 
                            VALUES ($1, $2) RETURNING *""", et.type_name, et.no_of_choices)
                        elif await self.get_election_type(connection=connection, type_id=et.type_id) is None:
                            row = await connection.fetchrow("""INSERT INTO election_type(type_id, type_name, 
                            no_of_choices) VALUES($1, $2, $3) RETURNING *""", et.type_id, et.type_name,
                                                            et.no_of_choices)
                        else:
                            # update
                            row = await connection.fetchrow("""UPDATE election_type SET type_name=$2, 
                            no_of_choices=$3 WHERE type_id=$1 RETURNING *""", et.type_id, et.type_name,
                                                            et.no_of_choices)
                except SerializationError:
                    await log('Retry {}'.format(retry))
                    continue
    ...
    async def get_voter(connection, voter_id: UUID) -> Voter | None:
        for retry in range(NO_OF_RETRIES):
            try:
                ...
                await log('Retry {}'.format(retry))
                continue
        return Voter(**dict(row)) if row else None
    ...
    async def add_voter_to_election(self, voter_id: UUID, election_id: UUID):
        for retry in range(NO_OF_RETRIES):
            async with self.pool.acquire() as connection:
                try:
                    if await self.is_voter_assigned_to_election(connection=connection, voter_id=voter_id,
                                                                election_id=election_id):
                        raise AccessDenied('Voter is already assigned to elections.')
                    async with connection.transaction(isolation=TRANSACTION_MODE):
    ...
    async def is_voter_assigned_to_election(connection, voter_id: UUID, election_id: UUID) -> bool:
        for retry in range(NO_OF_RETRIES):
            try:
                async with connection.transaction(isolation=TRANSACTION_MODE):
                    ans = await connection.fetchval("""SELECT EXISTS(SELECT 1 FROM voter_elections WHERE voter_id=$1
                    AND election_id=$2);""", voter_id, election_id)
            except SerializationError:
                await log('Retry {}'.format(retry))
                continue
        return ans
    ...
    async def get_election_dates(connection, election_id: UUID) -> tuple[datetime, datetime]:
        for retry in range(NO_OF_RETRIES):
            try:
                async with connection.transaction(isolation=TRANSACTION_MODE):
                    dates = await connection.fetchrow("""SELECT vote_start, vote_end FROM election WHERE
