...
class DbService:
    ...
    async def upsert_movie(self, movie: Movie) -> Movie:
        if movie.movie_id is None:
            # insert
            async with self.pool.acquire() as connection:
                row = await connection.fetchrow("insert into movies(title,budget,popularity,release_date,revenue) VALUES ($1,$1,$2,$3,$4,$5) returning *",
            ...
            # insert
            async with self.pool.acquire() as connection:
                row = await connection.fetchrow("insert into movies(movie_id,title,budget,popularity,release_date,revenue) VALUES ($1,$2,$3,$4,$5,$6) returning *",
