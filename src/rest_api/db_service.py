    ...
    async def upsert_lecture(self, lecture: Lecture) -> Lecture | None:
        async with self.pool.acquire() as connection:
            if lecture.lecture_id is None:
                row = await connection.fetchrow("""INSERT INTO lecture(lecture_name) VALUES ($1) RETURNING *""",
                                                lecture.lecture_name)
            elif await self.get_lecture(lecture.lecture_id) is None:
                row = await connection.fetchrow("""INSERT INTO lecture(lecture_id, lecture_name) VALUES($1, 
                $2) RETURNING *""", lecture.lecture_id, lecture.lecture_name)
            else:
                row = await connection.fetchrow("""UPDATE lecture SET lecture_name=$2 WHERE lecture_id=$1
                RETURNING *""", lecture.lecture_id, lecture.lecture_name)
            return Lecture(**dict(row)) if row else None
    ...
    async def upsert_lecturer(self, lecturer: Lecturer) -> Lecturer | None:
        async with self.pool.acquire() as connection:
            if lecturer.lecturer_id is None:
                row = await connection.fetchrow("""INSERT INTO lecturer(lecturer_name) VALUES ($1) RETURNING *""",
                                                lecturer.lecturer_name)
            elif await self.get_lecturer(lecturer.lecturer_id) is None:
                row = await connection.fetchrow("""INSERT INTO lecturer(lecturer_id, lecturer_name) VALUES($1, 
                $2) RETURNING *""", lecturer.lecturer_id, lecturer.lecturer_name)
            else:
                row = await connection.fetchrow("""UPDATE lecturer SET lecturer_name=$2 WHERE lecturer_id=$1 
                RETURNING *""", lecturer.lecturer_id, lecturer.lecturer_name)
    ...
    async def upsert_semester(self, semester: Semester) -> Semester | None:
        async with self.pool.acquire() as connection:
            if semester.semester_id is None:
                row = await connection.fetchrow("""INSERT INTO semester(semester_name, semester_start, semester_end) 
                VALUES ($1, $2, $3) RETURNING *""", semester.semester_name, semester.semester_start, 
                                                semester.semester_end)
            elif await self.get_semester(semester.semester_id) is None:
                row = await connection.fetchrow("""INSERT INTO semester(semester_id, semester_name, semester_start, 
                semester_end) VALUES($1, $2, $3, $4) RETURNING *""", semester.semester_id, semester.semester_name, 
                                                semester.semester_start, semester.semester_end)
            else:
                row = await connection.fetchrow("""UPDATE semester SET semester_name=$2, semester_start=$3, 
                semester_end=$4 WHERE semester_id=$1 RETURNING *""", semester.semester_id, semester.semester_name, 
                                                semester.semester_start, semester.semester_end)
            return Semester(**dict(row)) if row else None
    ...
    async def upsert_syllabus(self, syllabus: Syllabus) -> Syllabus | None:
        async with self.pool.acquire() as connection:
            if syllabus.syllabus_id is None:
                row = await connection.fetchrow("""INSERT INTO syllabus(lecture_id, semester_id, lecturer_id) VALUES 
                ($1, $2, $3) RETURNING *""", syllabus.lecture_id, syllabus.semester_id, syllabus.lecturer_id)
            elif await self.get_syllabus(syllabus.syllabus_id) is None:
                row = await connection.fetchrow("""INSERT INTO syllabus(syllabus_id, lecture_id, semester_id, 
                lecturer_id) VALUES ($1, $2, $3, $4) RETURNING *""", syllabus.syllabus_id, syllabus.lecture_id, 
                                                syllabus.semester_id, syllabus.lecturer_id)
            else:
                row = await connection.fetchrow("""UPDATE syllabus SET lecture_id=$2, semester_id=$3, lecturer_id=$4 
                WHERE syllabus_id=$1 RETURNING *""", syllabus.syllabus_id, syllabus.lecture_id, syllabus.semester_id,
    ...
    async def upsert_lecture_content(self, content: LectureContent) -> LectureContent | None:
        async with self.pool.acquire() as connection:
            if content.content_id is None:
                row = await connection.fetchrow("""INSERT INTO lecture_content(order_number, syllabus_id, 
                lecture_topic) VALUES ($1, $2, $3) RETURNING *""", content.order_number, content.syllabus_id, 
                                                content.lecture_topic)
            elif await self.get_syllabus(content.content_id) is None:
                row = await connection.fetchrow("""INSERT INTO lecture_content(content_id, order_number, syllabus_id, 
                lecture_topic) VALUES ($1, $2, $3, $4) RETURNING *""", content.syllabus_id, content.order_number, 
                                                content.syllabus_id, content.lecture_topic)
            else:
                row = await connection.fetchrow("""UPDATE lecture_content SET order_number=$2, syllabus_id=$3, 
                lecture_topic=$4 WHERE content_id=$1 RETURNING *""", content.syllabus_id, content.order_number, 
                                                content.syllabus_id, content.lecture_topic)
            return LectureContent(**dict(row)) if row else None
